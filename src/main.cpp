...
void processNewCard(const Card& new_card,
  ...
  if (is_last_card) {
    addNewStackLengthAndClear(finished_card_stack_lengths, current_card_stack);
  }
}
...
std::vector<long long> startSimulation() {
  ...
  long long cards_amount_in_simulation = safeInputs::safeBoundedNumberInput(
      "Enter cards amount in simulation in range [1, 9223372036854775807]: ", 1,
      LLONG_MAX);
  ...
  for (long long i = 0; i < cards_amount_in_simulation; i++) {
    ...
    processNewCard(next_card, finished_card_stack_lengths, current_card_stack,
                   i == cards_amount_in_simulation - 1);
  }
...
void outputStatistics(const std::vector<long long>& v) {
  ...
  std::cout << std::format("Median length: {:.1g}", median_length) << std::endl;
}
